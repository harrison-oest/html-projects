<!doctype html>
<html lang="en-US">
<head>
	<meta charset="UTF-8">
	<meta name="author" content="Harrison Oest" /> 
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<meta name="description" content="This page is all about my life." />
<title>All About Harrison Oest</title>
<link rel="stylesheet" type="text/css" href="csci445.css"/>
</head>

<?php $pageName = "index";?>
<?php $currentPath = ""; ?>

<body style="background-color: #a8d1f7">
	<?php include "templateHeader.php";?>
	<h1>Welcome to my page!</h1>
		<p>On this site, you can click the links located on the nav bar to learn more about me, as well as learn about HTML and CSS. Enjoy!</p>
		<p id="quiet_text">You're really getting a good look into my life, I hope you know that.</p>
	<footer id="foot">
		<?php include "templateFooter.php"; ?>
	</footer>
</body>
</html>
